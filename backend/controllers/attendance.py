...
async def get_contractor_from_user(user: User, db: AsyncSession) -> Contractor:
    ...
    result = await db.execute(
        select(Contractor).where(
            Contractor.person_phone == user.username
        )
    )
    ...
    if not contractor:
        raise HTTPException(
            ...
            detail="Contractor record not found for this user"
        )
...
async def log_attendance(
    ...
    try:
        ...
        return AttendanceResponse(
            ...
            block_name=full_attendance.village.block.name if full_attendance.village and full_attendance.village.block else None,
            district_name=full_attendance.village.district.name if full_attendance.village and full_attendance.village.district else None,
    ...
    try:
        ...
        # If user is an officer, check jurisdiction
            ...
            if not user_jurisdiction.get('is_admin'):
                if (user_jurisdiction.get('village_ids') and 
                    attendance.village_id not in user_jurisdiction['village_ids']):
                    raise HTTPException(
                        status_code=status.HTTP_403_FORBIDDEN,
                        detail="Attendance record is outside your jurisdiction"
                    )
                elif (user_jurisdiction.get('block_ids') and attendance.village and 
                      attendance.village.block_id not in user_jurisdiction['block_ids']):
                    raise HTTPException(
                        status_code=status.HTTP_403_FORBIDDEN,
                        detail="Attendance record is outside your jurisdiction"
                    )
                elif (user_jurisdiction.get('district_ids') and attendance.village and 
                      attendance.village.district_id not in user_jurisdiction['district_ids']):
                    raise HTTPException(
                        status_code=status.HTTP_403_FORBIDDEN,
                        detail="Attendance record is outside your jurisdiction"
                    )
