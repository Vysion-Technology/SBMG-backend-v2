...
async def update_role(
    ...
    try:
        user_mgmt_service = UserManagementService(db)
        role = await user_mgmt_service.update_role(
            role_id, request.name, request.description
        )
        if not role:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND, detail="Role not found"
    ...
    try:
        ...
        if not PermissionChecker.user_has_role(current_user, [UserRole.ADMIN]):
            if request.district_id:
                if not PermissionChecker.user_can_access_district(
                    current_user, request.district_id
                ):
                    raise HTTPException(
                        status_code=status.HTTP_403_FORBIDDEN,
                        detail="Cannot create user in this district",
                    )
...
async def get_all_position_holders(
    ...
    if role_name:
        positions = await user_mgmt_service.get_position_holders_by_role(
            role_name, skip, limit
        )
    else:
    ...
    for position in positions:
        ...
        if PermissionChecker.user_has_role(current_user, [UserRole.ADMIN]):
            can_access = True
        elif position.district_id and PermissionChecker.user_can_access_district(
            current_user, position.district_id
        ):
            can_access = True
        elif position.block_id and PermissionChecker.user_can_access_block(
            current_user, position.block_id
        ):
            can_access = True
        elif position.village_id and PermissionChecker.user_can_access_village(
            current_user, position.village_id
        ):
            can_access = True
...
async def get_position_holder(
    ...
    if not position:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="Position holder not found"
        )
...
async def update_position_holder(
    ...
    return PositionHolderResponse(
        ...
        district_name=updated_position.district.name
        if updated_position.district
        else None,
        ...
        village_name=updated_position.village.name
        if updated_position.village
        else None,
    )
...
async def change_user_password(
    ...
    try:
        user_mgmt_service = UserManagementService(db)
        success = await user_mgmt_service.change_user_password(
