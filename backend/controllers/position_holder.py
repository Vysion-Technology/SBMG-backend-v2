...
def validate_geographical_assignment(
    role_name: str, district_id: Optional[int], block_id: Optional[int], village_id: Optional[int]
):
    ...
    if role_name == UserRole.CEO:
        if not district_id or block_id or village_id:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST, detail="CEO must have district_id only (no block or village)"
            )
    elif role_name == UserRole.BDO:
        if not district_id or not block_id or village_id:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST, detail="BDO must have district_id and block_id (no village)"
            )
    elif role_name == UserRole.VDO:
        if not district_id or not block_id or not village_id:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST, detail="VDO must have district_id, block_id, and village_id"
            )


def validate_geographical_hierarchy(
    creator: User, district_id: Optional[int], block_id: Optional[int], village_id: Optional[int]
):
    ...
    if creator_role == UserRole.CEO:
        if creator.district_id != district_id:
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN, detail="CEO can only create positions within their own district"
            )
    ...
    # BDO can only assign within their block
        ...
        if creator.district_id != district_id or creator.block_id != block_id:
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN, detail="BDO can only create positions within their own block"
            )
    ...
    # VDO validation (if needed for future worker assignments)
        ...
        if creator.gp_id != village_id:
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN, detail="VDO can only create positions within their own village"
            )
...
async def create_position_holder(
    ...
    if not position:
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Failed to create position holder"
        )
...
async def get_position_holder(
    ...
    if not position:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail=f"Position holder with id {position_id} not found"
        )
    ...
    if creator_role not in [UserRole.SUPERADMIN, UserRole.ADMIN, UserRole.SMD]:
        if creator_role == UserRole.CEO:
            ...
            if district_id and district_id != current_user.district_id:
                raise HTTPException(
                    status_code=status.HTTP_403_FORBIDDEN, detail="CEO can only view positions within their district"
                )
            ...
            if district_id and district_id != current_user.district_id:
                raise HTTPException(
                    status_code=status.HTTP_403_FORBIDDEN, detail="BDO can only view positions within their district"
                )
            if block_id and block_id != current_user.block_id:
                raise HTTPException(
                    status_code=status.HTTP_403_FORBIDDEN, detail="BDO can only view positions within their block"
                )
            district_id = current_user.district_id
            block_id = current_user.block_id
        elif creator_role == UserRole.VDO:
            # VDO can only see positions in their village
            if gp_id and gp_id != current_user.gp_id:
                raise HTTPException(
                    status_code=status.HTTP_403_FORBIDDEN, detail="VDO can only view positions within their village"
                )
            district_id = current_user.district_id
            block_id = current_user.block_id
...
async def update_position_holder(
    ...
    if current_role_name and not can_create_role(creator_role, current_role_name):
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN, detail=f"{creator_role} cannot modify {current_role_name} positions"
        )
    ...
    if request.role_name:
        if not can_create_role(creator_role, request.role_name):
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN, detail=f"{creator_role} cannot assign {request.role_name} role"
            )
        ...
        new_village = request.village_id if request.village_id is not None else existing_position.gp_id
    ...
    if not updated_position:
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Failed to update position holder"
        )
    ...
    return PositionHolderResponse(
        ...
        village_id=updated_position.gp_id,
...
async def delete_position_holder(
    ...
    if current_role_name and not can_create_role(creator_role, current_role_name):
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN, detail=f"{creator_role} cannot delete {current_role_name} positions"
        )
    ...
    if not deleted:
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Failed to delete position holder"
        )
