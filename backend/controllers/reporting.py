...
from auth_utils import (
    require_staff_role, UserRole, PermissionChecker
)
...
# Worker endpoints
@router.get("/worker/assigned-complaints", response_model=List[AssignedComplaintResponse])
async def get_worker_assigned_complaints(
    ...
    if not PermissionChecker.user_has_role(current_user, [UserRole.WORKER, UserRole.VDO, UserRole.BDO, UserRole.CEO, UserRole.ADMIN]):
        raise HTTPException(
    ...
    if current_user.positions and any(pos.role.name == UserRole.WORKER for pos in current_user.positions):
    ...
    elif current_user.positions and any(pos.role.name == UserRole.VDO for pos in current_user.positions):
        # VDOs can see all complaints in their village
        vdo_villages = [pos.village_id for pos in current_user.positions if pos.role.name == UserRole.VDO and pos.village_id]
        if not vdo_villages:
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN,
        ...
        complaints = result.scalars().all()
    elif current_user.positions and any(pos.role.name == UserRole.BDO for pos in current_user.positions):
        # BDOs can see all complaints in their block
        bdo_blocks = [pos.block_id for pos in current_user.positions if pos.role.name == UserRole.BDO and pos.block_id]
        if not bdo_blocks:
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN,
        ...
        complaints = result.scalars().all()
    elif current_user.positions and any(pos.role.name == UserRole.CEO for pos in current_user.positions):
        # CEOs can see all complaints in their district
        ceo_districts = [pos.district_id for pos in current_user.positions if pos.role.name == UserRole.CEO and pos.district_id]
        ...
        complaints = result.scalars().all()
    elif current_user.positions and any(pos.role.name == UserRole.ADMIN for pos in current_user.positions):
...
@router.post("/worker/complaints/{complaint_id}/media", response_model=MediaUploadResponse)
...
async def verify_complaint(
    ...
    if not PermissionChecker.user_can_access_village(current_user, complaint.village_id):
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
    ...
    if UserRole.ADMIN not in user_roles:
        ...
        for position in current_user.positions:
            if position.role.name == UserRole.CEO and position.district_id:
                jurisdiction_filter.append(Complaint.district_id == position.district_id)
...
async def get_complaint_details(
    ...
    if UserRole.ADMIN not in user_roles:
        has_access = False
        for position in current_user.positions:
            if position.role.name == UserRole.CEO and position.district_id == complaint.district_id:
                has_access = True
                break
            elif position.role.name == UserRole.BDO and position.block_id == complaint.block_id:
                has_access = True
                break
            elif position.role.name == UserRole.VDO and position.village_id == complaint.village_id:
                has_access = True
                break
