...
async def get_unified_dashboard(
    ...
    for complaint in recent_complaints_data:
        ...
        recent_complaints.append(ComplaintResponse(
            id=complaint.id,
            description=complaint.description,
            status_name=complaint.status.name,
            complaint_type_name=complaint.complaint_type.name if complaint.complaint_type else None,
    ...
    if any(role in [UserRole.ADMIN, UserRole.CEO, UserRole.BDO] for role in user_roles):
        ...
        if UserRole.ADMIN in user_roles:
            ...
            ceo_districts = [pos.district_id for pos in current_user.positions 
                           if pos.role.name == UserRole.CEO and pos.district_id]
            if ceo_districts:
                block_count = await db.execute(
                    select(func.count(Block.id)).where(Block.district_id.in_(ceo_districts))
                )
                village_count = await db.execute(
                    select(func.count(Village.id)).where(Village.district_id.in_(ceo_districts))
                )
                geographic_summary = {
...
async def get_complaints(
    ...
    date_from: Optional[date] = Query(None, description="Filter complaints from date (YYYY-MM-DD)"),
    date_to: Optional[date] = Query(None, description="Filter complaints to date (YYYY-MM-DD)"),
    search: Optional[str] = Query(None, description="Search in complaint description"),
    sort_by: str = Query("created_at", description="Sort by field (created_at, updated_at, status)"),
...
async def get_worker_tasks(
    ...
    for complaint in complaints:
        ...
        tasks.append(WorkerTaskResponse(
            ...
            media_urls=[media.media_url for media in complaint.media],
            completion_percentage=75 if complaint.status.name == "IN_PROGRESS" else 
                                 100 if complaint.status.name in ["COMPLETED", "VERIFIED"] else 0
        ))
...
async def get_admin_analytics(
    ...
    total_villages = await db.execute(select(func.count(Village.id)))
    ...
    user_productivity_query = (
        select(
            User.username,
            func.count(ComplaintAssignment.id).label("assigned_tasks")
        )
        .select_from(User)
