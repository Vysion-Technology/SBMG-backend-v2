...
class S3Service:
    ...
    def __init__(self):
        ...
        if settings.s3_endpoint_url and settings.s3_access_key and settings.s3_secret_key:
            try:
                ...
                logger.info("S3 client initialized with endpoint: %s", settings.s3_endpoint_url)
    ...
    def is_available(self) -> bool:
        ...
        return self.client is not None  # type: ignore
    ...
    async def upload_file(self, file: UploadFile, folder: str = "uploads", filename: Optional[str] = None) -> str:
        ...
        try:
            ...
            # Upload to S3
            self.client.put_object(  # type: ignore
    ...
    def download_file(self, s3_key: str) -> bytes:
        ...
        try:
            response = self.client.get_object(Bucket=self.bucket_name, Key=s3_key)  # type: ignore
    ...
    def delete_file(self, s3_key: str) -> bool:
        ...
        try:
            self.client.delete_object(Bucket=self.bucket_name, Key=s3_key)  # type: ignore
            ...
            logger.error("Unexpected delete error: %s", e)
            return False
