...
from models.response.attendance import AttendanceResponse, AttendanceListResponse, AttendanceSummaryResponse, AttendanceStatsResponse
...
class AttendanceService:
    ...
    async def log_attendance(self, contractor_id: int, request: AttendanceLogRequest) -> Attendance:
        ...
        existing_attendance = await self.db.execute(
            ...
            .where(
                and_(
                    Attendance.contractor_id == contractor_id,
    ...
    async def end_attendance(self, contractor_id: int, request: AttendanceEndRequest) -> Attendance:
    ...
    async def get_attendance_by_id(self, attendance_id: int) -> Optional[Attendance]:
        ...
        result = await self.db.execute(
            select(Attendance)
            .options(
                joinedload(Attendance.contractor).joinedload(Contractor.agency),
                joinedload(Attendance.village).joinedload(Village.block).joinedload(Block.district)
            )
            .where(Attendance.id == attendance_id)
        )
        return result.scalar_one_or_none()

    async def get_contractor_attendances(self, contractor_id: int, start_date: Optional[date] = None, end_date: Optional[date] = None) -> List[Attendance]:
        """Get attendance records for a specific contractor"""
        query = select(Attendance).where(Attendance.contractor_id == contractor_id)
    ...
    async def get_filtered_attendances(self, filters: AttendanceFilterRequest, user_jurisdiction: Dict[str, Any]) -> AttendanceListResponse:
        ...
        if user_jurisdiction.get('village_ids'):
            query = query.where(Attendance.village_id.in_(user_jurisdiction['village_ids']))
        elif user_jurisdiction.get('block_ids'):
            # Join with village to filter by block
            query = query.join(Village).where(Village.block_id.in_(user_jurisdiction['block_ids']))
        elif user_jurisdiction.get('district_ids'):
            # Join with village to filter by district
            query = query.join(Village).where(Village.district_id.in_(user_jurisdiction['district_ids']))

        # Apply additional filters
        if filters.contractor_id:
            query = query.where(Attendance.contractor_id == filters.contractor_id)
        if filters.village_id:
            query = query.where(Attendance.village_id == filters.village_id)
        if filters.start_date:
            query = query.where(Attendance.date >= filters.start_date)
        if filters.end_date:
            query = query.where(Attendance.date <= filters.end_date)
        ...
        query = query.order_by(Attendance.date.desc(), Attendance.start_time.desc())
        ...
        for attendance in attendances:
            attendance_responses.append(AttendanceResponse(
                ...
                district_name=attendance.village.district.name if attendance.village and attendance.village.district else None,
    ...
    async def get_attendance_stats(self, user_jurisdiction: Dict[str, Any], start_date: Optional[date] = None, end_date: Optional[date] = None) -> AttendanceStatsResponse:
        ...
        contractors_query = select(Contractor).options(
            joinedload(Contractor.village).joinedload(Village.block).joinedload(Block.district)
        )
        ...
        if user_jurisdiction.get('village_ids'):
            ...
            contractors_query = contractors_query.join(Village).where(Village.block_id.in_(user_jurisdiction['block_ids']))
        elif user_jurisdiction.get('district_ids'):
            contractors_query = contractors_query.join(Village).where(Village.district_id.in_(user_jurisdiction['district_ids']))
        ...
        today = date.today()
        present_today_query = select(func.count(func.distinct(Attendance.contractor_id))).where(
            and_(
                Attendance.contractor_id.in_(contractor_ids),
    ...
    async def _get_contractor_summary(self, contractor: Contractor, start_date: Optional[date] = None, end_date: Optional[date] = None) -> AttendanceSummaryResponse:
        ...
        """)
        total_days_result = await self.db.execute(total_days_query, {
        ...
        # Count present days
        present_days_query = select(func.count(Attendance.id)).where(
            and_(
                Attendance.contractor_id == contractor.id,
        ...
        last_attendance_query = select(func.max(Attendance.date)).where(
            Attendance.contractor_id == contractor.id
        )
