...
class GeographyService:
    ...
    async def check_village_name_unique_in_gp(
        ...
        query = select(Village).where(Village.name == name, Village.gp_id == gp_id)
        if exclude_id:
    ...
    async def can_delete_village(self, village_id: int) -> bool:
        ...
        # Check for complaints
        complaints_result = await self.db.execute(select(func.count(Complaint.id)).where(Complaint.gp_id == village_id))
        complaints_count = complaints_result.scalar() or 0
    ...
    async def get_village_by_id(self, village_id: int) -> Village:
        ...
        result = await self.db.execute(
            select(Village).options(selectinload(Village.gram_panchayat)).where(Village.id == village_id)
        )
        village = result.scalar_one_or_none()
        if not village:
    ...
    async def list_village_entities(
        self,
        block_id: Optional[int] = None,
        district_id: Optional[int] = None,
        gp_id: Optional[int] = None,
    ) -> list[Village]:
        """List all village entities, optionally filtered by block or district."""
        query = select(Village).order_by(Village.name)
        if block_id:
            query = query.join(GramPanchayat, Village.gp_id == GramPanchayat.id).where(
    ...
    async def create_village(self, village_req: CreateVillageRequest) -> Village:
        ...
        is_unique = await self.check_village_name_unique_in_gp(village_req.name, village_req.gp_id)
        if not is_unique:
            raise HTTPException(
                status_code=400,
    ...
    async def create_bulk_villages(
        self,
        create_villages: List[CreateVillageRequest],
    ) -> List[Village]:
        """Create bulk villages in the villages table."""
        villages_queries = await self.db.execute(
            insert(Village)
            .returning(Village)
            .values(
                [
                    ...
                    for village in create_villages
                ],
            )
        )
        await self.db.commit()
        villages = villages_queries.scalars().all()
