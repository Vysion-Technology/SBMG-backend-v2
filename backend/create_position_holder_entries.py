...
async def create_users(db: AsyncSession, dry_run: bool, roles_map: Dict[str, Role], users: List[User]) -> tuple[int, int, int]:
    created_count = 0
    skipped_count = 0
    error_count = 0
    ...
    for user in users:
        try:
            ...
            existing_ph = await db.execute(
                select(PositionHolder).where(PositionHolder.user_id == user.id)
            )
            ...
            if dry_run:
                print(f"  [DRY RUN] Would create PositionHolder for user {user.username} (ID: {user.id}) with role {user_role.value}")
            else:
                ...
                print(f"  ✓ Created PositionHolder for user {user.username} (ID: {user.id}) - Role: {user_role.value} - {geo_str}")
...
async def insertion(db: AsyncSession, user, role, first_name, last_name):
    await db.execute(
